...
<body>
	...
	<form action="check" name="form" method="post"
		...
		<div class="container">
			...
			<div class="row">
				...
				DOB :<span id="errorDob" style="color: red"></span><input
			...
			<div class="row">
				...
				Location <select class="form-select" name="location" id="location"
			...
			<div class="row">
				Password <input type="password" name="password" id="password"
					class="form-control" onchange="passwordHandle()" /> <span
			...
			<div class="row">
				Confirm Password <input type="password" name="confirmPassword"
					...
					onchange="passwordHandle()" /><span id="errorConfirm"
					style="color: red"></span>
			</div>
			<br>
	...
	function validateForm(){
		...
		if (name.length < 4 || name.length > 20) {
				...
				if(dob === ' '){
					document.getElementById("errorDob").innerHTML = "select date...";
				}
				else{
				...
				if (location === "--Select--") {
				document.getElementById("errorLocation").innerHTML = "Select here.."
				...
				if (confirmPassword !== password) {
					document.getElementById("errorConfirm").innerHTML = "Password not matching.."
						return false;
				}
				else{
		...
		function mobileHandle(event) {
			...
			if (mobile > 1000000000 && mobile.length == 10 && mobile < 9999999999) {
				document.getElementById("errorMobile").innerHTML = "";
				btn.removeAttribute("disabled");
	...
	/*	function emailHandle(event) {
		...
		function passwordHandle(event) {
			...
			if (password.length > 8 && password.length < 13 && confirmPassword === password) {
				document.getElementById("errorPassword").innerHTML = ""
				btn.removeAttribute("disabled")
			} 
			
			else {
				document.getElementById("errorPassword").innerHTML = "Please enter the password here..";
				btn.setAttribute("disabled", "");
				return;
			}
